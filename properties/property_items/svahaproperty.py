...
class SvahaPropertyItem(scrapy.Item):
    ...
    list_date = scrapy.Field(
        input_processor=MapCompose(get_uploaded_date),
    ...
    years = scrapy.Field(
        output_processor=FindLeaseYears(),
    )
    ...
    price_usd = scrapy.Field(output_processor=TakeFirst())
    property_link = scrapy.Field(output_processor=TakeFirst())
    image = scrapy.Field(
        input_processor=MapCompose(dimension_remover),
        output_processor=TakeFirst(),
    )
    availability = scrapy.Field(output_processor=TakeFirst())
    description = scrapy.Field(output_processor=JoinAndStrip("\n"))
